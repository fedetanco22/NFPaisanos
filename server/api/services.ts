...
export const getAuctions = async (): Promise<NFPaisanos[]> => {
  const response = await axios.get(`${API_URL}/nfpaisanos/aunctions`, headers)
  console.log('🚀 ~ file: services.ts:35 ~ getAuctions ~ response:', response)
  return response.data
  // return new Promise(resolve => {
  //   setTimeout(() => {
  //     resolve([
...
export const getPopularAuctions = async (): Promise<NFPaisanos[]> => {
  // const response = await axios.get(`${API_URL}/nfpaisanos/popular`, headers)
  // return response.data
  return new Promise(resolve => {
    setTimeout(() => {
      resolve([
